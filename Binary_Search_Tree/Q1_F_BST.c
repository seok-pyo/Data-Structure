...
{
	// Queue *q; 차이점 익히기.
	Queue q;			 // 여기서는 포인터로 받아올 이유가 없음.
	q.head = NULL; // q 초기화
	q.tail = NULL;
	enqueue(&(q.head), &(q.tail), root); // & 필요?

	while (!isEmpty(q.head))
	{
		BSTNode *ele = dequeue(&(q.head), &(q.head));
		if (ele != NULL)
		{
			printf("%d ", ele->item);
			enqueue(&(q.head), &(q.tail), ele->left);
			enqueue(&(q.head), &(q.tail), ele->right);
		}
	}
}
